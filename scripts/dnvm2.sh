...
_DNVM_RUNTIME_FRIENDLY_NAME="dotnet CLI"
_DNVM_RUNTIME_SHORT_NAME="dotnet CLI"
...
_DNVM_USER_PACKAGES="$DOTNET_USER_HOME/runtimes"
_DNVM_GLOBAL_PACKAGES="$DOTNET_GLOBAL_HOME/runtimes"
...
__dnvm_locate_runtime_bin_from_full_name() {
    ...
    for v in `echo $DOTNET_HOME | tr ":" "\n"`; do
        if [ -e "$v/runtimes/$runtimeFullName" ]; then
            echo "$v/runtimes/$runtimeFullName" && return
        fi
    done
}
